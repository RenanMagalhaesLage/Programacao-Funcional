...
listaVendas :: Dia -> [Int]
listaVendas 0 = []
listaVendas dia = (vendas dia) : (listaVendas (dia-1))
...
type Quant = Int
listaVendasDefinidas :: Quant -> Dia -> [Int]
listaVendasDefinidas quant dia
  |dia <= 0 = []
  |quant == vendas dia = (dia) : (listaVendasDefinidas quant (dia-1))
  |otherwise = listaVendasDefinidas quant (dia-1)
...
listaVendasSuperior :: Quant -> Dia -> [Int]
listaVendasSuperior quant dia
  |dia <= 0 = []
  |quant < vendas dia = (dia) : (listaVendasSuperior quant (dia-1))
  |otherwise = listaVendasSuperior quant (dia-1)
